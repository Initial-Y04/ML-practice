...
print(tree_rmse)   # tree_rmse = 10.05
...
print(forest_rmse)  # forest_rmse = 37.43

# 使用交叉验证评估三种模型：随机森林表现最好
scores = cross_val_score(tree_reg,X_train,y_train,scoring='neg_mean_squared_error',cv=10)
rmse_scores = np.sqrt(-scores)
print(rmse_scores.mean())  # 113.06

lin_scores = cross_val_score(lin_reg,X_train,y_train,scoring='neg_mean_squared_error',cv=10)
lin_rmse_scores = np.sqrt(-lin_scores)
print(lin_rmse_scores.mean())  # 109.19

forest_scores = cross_val_score(forest_reg,X_train,y_train,scoring='neg_mean_squared_error',cv=10)
forest_rmse_scores = np.sqrt(-forest_scores)
print(forest_rmse_scores.mean())  # 88.41
...
print(final_rmse)   # 80.18
print(final_model.score(X_test,y_test))  # accuracy_score = 0.87
